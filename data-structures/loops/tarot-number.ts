...
validar que la fecha ingresada sea correcta, no puedes recibir una fecha como 58/99/9999.

Aunque sí podrías recibir una fecha como 12/12/1000 -> 7, ya que podría ser alguien que esté usando tu código! ;)
*/
...
// const birthDate: string = prompt('Ingrese fecha de nacimiento en formato dia/mes/año: ')

// // export const getSpecialNumber = (date:string): number =>{
//     let specialNumber = 0
//     const [dayString, monthString, yearString] = date.split("/")
//     const day = parseInt(dayString)
//     const month = parseInt (monthString)
//     const year = parseInt (yearString)
//     for (let i = 1; i < (date.length-2); i++){
//         const firstStep = date[i]
//     }
...
const calculateTarotNumber = (day:number, month:number, year:number): number| null => {
    ...
    while (total >= MAX_TAROT_NUMBER){
        total = sumDigits(total)
    }
    return total
}

console.log(calculateTarotNumber(26,10,1983))